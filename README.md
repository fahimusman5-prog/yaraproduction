...
3. Set `NEXT_PUBLIC_SITE_URL` (or the legacy `NEXT_PUBLIC_APP_URL`) to the canonical HTTPS production origin. PayHere posts verified notifications to `/api/payments/payhere/notify`.
4. Confirm the merchant account accepts LKR. UAE card checkout requires a current approved AED→LKR rate.
...
For UAE PayHere checkout, configure a current approved AED→LKR rate in Admin
Commerce. The commercial order remains in AED while the payment attempt locks
the disclosed LKR charge. This implementation never submits AED or INR to
PayHere.
