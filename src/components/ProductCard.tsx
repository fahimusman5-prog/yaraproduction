  ...
  return (
    <article className="group overflow-hidden rounded-[1.8rem] bg-white shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-soft">
      <div className="relative aspect-[1/1.02] overflow-hidden bg-yara-rose">
        <Link to={productPath} aria-label={t("common.viewProduct", { name: product.name })}>
        ...
        <button
          ...
          aria-label={saved ? t("product.removeFavorite", { name: product.name }) : t("product.addFavorite", { name: product.name })}
        >
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          ...
          <button onClick={handleAdd} disabled={product.stockQuantity === 0} className="grid h-10 w-10 place-items-center rounded-full bg-yara-wine text-white transition hover:scale-105 disabled:cursor-not-allowed disabled:opacity-40" aria-label={t("product.addNamedToCart", { name: product.name })}>
