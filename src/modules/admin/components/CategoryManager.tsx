...
function CategoryRow({ category }: { category: Category }) {
  ...
  return <tr><td><p className="font-semibold">{category.name}</p><p className="mt-1 font-mono text-xs text-slate-400">{category.slug}</p></td><td><StatusBadge value={category.status} /></td><td><div className="flex justify-end gap-2"><button className="staff-button staff-button-secondary" onClick={() => dialogRef.current?.showModal()} aria-label={`Edit ${category.name}`}><Pencil className="h-4 w-4" /></button><ConfirmActionButton action={deleteCategoryAction.bind(null, category.id)} label="Delete" title={`Delete ${category.name}?`} detail="Products in this category will become uncategorized. This cannot be undone." /></div><dialog ref={dialogRef} className="staff-dialog"><div className="p-6 sm:p-8"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Edit category</h2><button onClick={() => dialogRef.current?.close()} className="grid min-h-11 min-w-11 place-items-center" aria-label="Close"><X className="h-5 w-5" /></button></div><div className="mt-5"><CategoryForm category={category} close={() => dialogRef.current?.close()} /></div></div></dialog></td></tr>;
}
