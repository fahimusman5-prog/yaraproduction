    ...
    <div className="page-shell py-12 sm:py-20">
      ...
      >
        ...
        <aside className="surface-card p-6 sm:p-8 lg:sticky lg:top-28">
          ...
          {payHereSubmission ? (
            <div>
              <label className="mt-5 flex items-start gap-3 text-xs leading-5 text-yara-taupe"><input type="checkbox" name={PAYHERE_ACKNOWLEDGEMENT_FIELD} required checked={payHereAcknowledged} onChange={(event) => { const acknowledged = event.currentTarget.checked; setPayHereFlow((current) => setPayHereAcknowledged(current, acknowledged)); }} disabled={submitting} className="mt-0.5 h-4 w-4 accent-yara-wine" /> I have reviewed the AED order total and the exact LKR PayHere charge above.</label>
