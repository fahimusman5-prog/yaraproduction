...
export function CheckoutPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (payment === "cod") trackEvent("cod_order_completed", { country, currency: country === "sri-lanka" ? "LKR" : "AED", value: Number(result.totalAmount ?? total ?? 0), order_id: result.orderId });
      clearCart();
  ...
  const handleWhatsAppOrder = () => {
    ...
    window.open(createWhatsAppLink(cartOrderMessage(items, confirmedTotal, country, { ...customer, paymentMethod: t(payment === "payhere" ? "checkout.payhere" : "checkout.cod") }, locale, {
      subtotal,
    ...
    <div className="page-shell py-12 sm:py-20">
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="mt-10 grid items-start gap-6 lg:grid-cols-[1fr_390px] xl:gap-10">
        <div className="space-y-6">
          ...
          <section className="surface-card p-6 sm:p-8">
            ...
            <div className="mt-7 space-y-3">
              <label className={`block rounded-[1.7rem] border p-5 transition ${paymentsEnabled && delivery.configured && delivery.enabled ? "cursor-pointer" : "cursor-not-allowed opacity-70"} ${payment === "payhere" ? "border-yara-wine bg-yara-blush" : "border-yara-rose"}`}>
                <span className="flex items-center gap-3 text-sm"><input type="radio" name="payment" checked={payment === "payhere"} onChange={() => setPayment("payhere")} disabled={!paymentsEnabled || !delivery.configured || !delivery.enabled} className="accent-yara-wine" /><CreditCard className="h-4 w-4 text-yara-wine" /> {t("checkout.payhere")}</span>
                <span className="mt-3 block text-xs leading-5 text-yara-taupe">{!delivery.configured || !delivery.enabled ? "Online payment is unavailable until the regional delivery fee is configured." : paymentsEnabled ? t("checkout.payhereCopy") : "Online payments are temporarily unavailable while the payment provider is being activated."}</span>
              </label>
              <label className={`flex items-center gap-3 rounded-full border p-5 text-sm transition ${!delivery.configured || !delivery.enabled || selectedShipping?.codAvailable === false ? "cursor-not-allowed opacity-60" : "cursor-pointer"} ${payment === "cod" ? "border-yara-wine bg-yara-blush" : "border-yara-rose"}`}><input type="radio" name="payment" checked={payment === "cod"} onChange={() => setPayment("cod")} disabled={!delivery.configured || !delivery.enabled || selectedShipping?.codAvailable === false} className="accent-yara-wine" /><Banknote className="h-4 w-4 text-yara-wine" /> {t("checkout.cod")}</label>
            </div>
          </section>
        </div>
        ...
        <aside className="surface-card p-6 sm:p-8 lg:sticky lg:top-28">
          ...
          {country === "uae" && (!delivery.configured || !delivery.enabled) && <p role="status" className="mt-4 rounded-2xl bg-amber-50 p-3 text-sm leading-6 text-amber-900">UAE delivery is not configured. Your delivery fee will be confirmed through WhatsApp before payment.</p>}
          ...
          <button type="submit" disabled={submitting || !hasLiveCatalogItems || !delivery.configured || !delivery.enabled || total === null || !selectedShipping || (payment === "cod" && !selectedShipping.codAvailable) || unavailableProductIds.length > 0} className="btn-primary mt-7 w-full" aria-busy={submitting}>{submitting ? <LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" /> : <ArrowRight className="h-4 w-4" aria-hidden="true" />}{submitting ? t("checkout.preparing") : t("checkout.confirm")}</button>
