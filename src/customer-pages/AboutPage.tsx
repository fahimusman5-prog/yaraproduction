import { ArrowRight, Award, BookOpen, Globe2, Leaf, Medal, Sparkles, Check } from "lucide-react";
import { motion, useReducedMotion } from "framer-motion";
import Image from "next/image";
import { Link } from "react-router-dom";
import { founderStory, type EducationItem, type FounderAward } from "../data/founder-story";
...
function EducationIcon({ icon }: { icon: EducationItem["icon"] }) {
  const Icon = { book: BookOpen, leaf: Leaf, sparkle: Sparkles, globe: Globe2, certificate: Medal }[icon];
...
export function AboutPage() {
  return (
    <div className="overflow-hidden bg-[#fffdfa] text-yara-ink">
      ...
      <section aria-labelledby="education-title" className="bg-[#fbf2f0] py-16 sm:py-20 lg:py-24"><div className="page-shell"><Reveal className="max-w-3xl"><p className="eyebrow">{founderStory.education.label}</p><h2 id="education-title" className="mt-4 text-balance text-4xl leading-tight sm:text-5xl">{founderStory.education.title}</h2><p className="mt-5 max-w-2xl text-base font-light leading-7 text-yara-taupe sm:text-lg sm:leading-8">{founderStory.education.introduction}</p></Reveal><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{founderStory.education.items.map((item, index) => <Reveal key={item.name} className="h-full" delay={Math.min(index * 0.04, 0.2)}><article className="flex h-full min-h-[12.5rem] flex-col rounded-[1.5rem] border border-yara-gold/20 bg-white/80 p-5 shadow-[0_12px_28px_rgba(91,34,53,.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_36px_rgba(91,34,53,.1)] sm:p-6"><span className="grid h-10 w-10 place-items-center rounded-full border border-yara-gold/25 bg-yara-blush text-yara-wine"><EducationIcon icon={item.icon} /></span><h3 className="mt-5 text-lg leading-tight text-yara-wine">{item.name}{item.flag && <span className="ml-2" role="img" aria-label="Malaysia">{item.flag}</span>}</h3><p className="mt-3 text-sm font-light leading-6 text-yara-taupe">{item.description}</p></article></Reveal>)}</div></div></section>
