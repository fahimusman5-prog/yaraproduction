...
export function AboutPage() {
  ...
  const values = list("about.values").map((item, index) => {
    const [title, text] = item.split("|");
    return { title, text, icon: [HeartHandshake, ShieldCheck, Sparkles][index] ?? Sparkles };
  });
  return (
    <>
      <section className="page-shell grid items-center gap-12 py-14 sm:py-20 lg:grid-cols-2 lg:py-24">
        <div className="max-w-xl">
          ...
          <h1 className="mt-4 text-balance text-5xl leading-[1.08] sm:text-6xl">{t("about.title")}</h1>
      ...
      <section className="bg-yara-blush py-20 sm:py-28">
        <div className="page-shell text-center">
          <p className="eyebrow">{t("about.mission")}</p><h2 className="mx-auto mt-4 max-w-3xl text-balance text-4xl sm:text-5xl">{t("about.missionTitle")}</h2>
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {values.map(({ icon: Icon, title, text }) => <div key={title} className="surface-card p-8"><span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-yara-rose text-yara-wine"><Icon className="h-6 w-6" /></span><h3 className="mt-6 text-2xl">{title}</h3><p className="mt-4 text-sm font-light leading-7 text-yara-taupe">{text}</p></div>)}
          </div>
        </div>
      </section>
      ...
      <section className="page-shell py-20 sm:py-28">
        <div className="grid items-center gap-10 rounded-[2.5rem] border border-yara-gold/30 bg-white p-7 shadow-card sm:p-12 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <div><p className="eyebrow">{t("about.regionEyebrow")}</p><h2 className="mt-4 text-4xl sm:text-5xl">{t("about.regionTitle")}</h2><p className="mt-6 text-sm font-light leading-7 text-yara-taupe">{t("about.regionCopy")}</p><div className="mt-7 grid gap-3 sm:grid-cols-2">{list("about.regionPoints").map((item) => <p key={item} className="flex items-center gap-3 text-sm"><CheckCircle2 className="h-5 w-5 text-yara-wine" />{item}</p>)}</div></div>
        </div>
      </section>

      <section className="page-shell"><div className="overflow-hidden rounded-[2.5rem] bg-gradient-to-r from-yara-wine to-[#7f153c] px-6 py-14 text-center text-white sm:px-12 sm:py-20"><h2 className="text-balance text-4xl sm:text-5xl">{t("about.promiseTitle")}</h2><p className="mx-auto mt-5 max-w-2xl text-sm font-light leading-7 text-white/75">{t("about.promiseCopy")}</p><div className="mt-8 flex flex-wrap justify-center gap-3"><Link to="/shop" className="btn-secondary border-white bg-white text-yara-wine">{t("shop.title")}</Link><Link to="/contact" className="inline-flex min-h-11 items-center justify-center rounded-full border border-white/50 px-6 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-white">{t("layout.contactUs")}</Link></div></div></section>
    </>
  );
}
