...
const categories: Array<"All" | Category> = ["All", "Skincare", "Haircare", "Body Rituals", "Gift Sets"];
  ...
  const filters = (
    <div className="space-y-8">
      ...
      <div className="rounded-[1.7rem] bg-yara-rose p-6">
        <p className="font-serif text-lg">Join the Inner Circle</p><p className="mt-2 text-xs font-light leading-5 text-yara-taupe">Early access to rituals and private offers.</p><button className="btn-primary mt-5 w-full">Subscribe</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="page-shell py-12 sm:py-16 lg:py-20">
      <div className="mx-auto max-w-3xl text-center">
        <p className="eyebrow">The ritual edit</p><h1 className="mt-4 text-4xl sm:text-5xl">Elevate Your Ritual</h1><p className="mt-5 text-sm font-light leading-7 text-yara-taupe">Scientifically advanced formulas meet soul-nurturing luxury. Discover our curated collection of botanical excellence.</p>
      </div>
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-[220px_1fr] xl:grid-cols-[250px_1fr]">
        ...
        <div>
          <div className="mb-7 flex items-center justify-between gap-3 rounded-full bg-white px-5 py-3.5 shadow-card sm:px-6">
            <p className="text-xs italic text-yara-taupe">Showing {visibleProducts.length} essential ritual{visibleProducts.length === 1 ? "" : "s"}</p>
          ...
          {visibleProducts.length ? (
            ...
            <div className="surface-card py-24 text-center"><h2 className="text-3xl">No ritual found</h2><p className="mt-3 text-sm text-yara-taupe">Try a different category or search phrase.</p><button onClick={() => { updateCategory("All"); setConcern("All"); clearSearch(); }} className="btn-primary mt-6">Reset filters</button></div>
          )}
        </div>
      </div>
      ...
      {filtersOpen && (
        <div className="fixed inset-0 z-[60] bg-yara-ink/25 backdrop-blur-sm lg:hidden" onClick={() => setFiltersOpen(false)}>
          <aside className="ml-auto h-full w-[88%] max-w-sm overflow-y-auto bg-yara-ivory p-7" onClick={(event) => event.stopPropagation()}>
            <div className="mb-8 flex items-center justify-between"><h2 className="text-2xl">Refine your ritual</h2><button onClick={() => setFiltersOpen(false)} aria-label="Close filters"><X className="h-5 w-5" /></button></div>
            {filters}
