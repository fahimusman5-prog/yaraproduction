  ...
  if (!items.length) {
    return (
      <div className="page-shell py-24 text-center sm:py-36">
        ...
        <p className="eyebrow mt-7">Your ritual awaits</p><h1 className="mt-3 text-4xl sm:text-5xl">Your bag is beautifully empty.</h1><p className="mx-auto mt-4 max-w-md text-sm font-light leading-7 text-yara-taupe">Explore our formulas and begin building a ritual that feels entirely your own.</p><Link to="/shop" className="btn-primary mt-8">Discover the collection <ArrowRight className="h-4 w-4" /></Link>
      </div>
    );
  }
    ...
    <div className="page-shell py-12 sm:py-20">
      ...
      <div className="mt-10 grid items-start gap-10 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_420px]">
            ...
            <article key={product.id} className="surface-card grid grid-cols-[100px_1fr] gap-4 p-4 sm:grid-cols-[150px_1fr] sm:gap-6 sm:p-5">
              ...
              <div className="flex min-w-0 flex-col justify-between py-1">
                ...
                <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <p className="font-serif text-xl font-semibold text-yara-wine">{formatPrice(product.price * quantity)}</p>
                </div>
              </div>
            </article>
          ))}
        ...
        <aside className="surface-card p-6 sm:p-8 lg:sticky lg:top-28">
          ...
          <div className="mt-6 space-y-4 border-b border-yara-rose pb-6 text-sm"><div className="flex justify-between"><span className="text-yara-taupe">Subtotal</span><span>{formatPrice(subtotal)}</span></div><div className="flex justify-between"><span className="text-yara-taupe">Shipping</span><span>{shipping ? formatPrice(shipping) : "Complimentary"}</span></div></div>
          {subtotal < 75 && <p className="mt-4 rounded-2xl bg-yara-blush p-4 text-xs leading-5 text-yara-taupe">You’re {formatPrice(75 - subtotal)} away from complimentary shipping.</p>}
          <div className="mt-6 flex items-end justify-between"><span className="font-serif text-2xl">Total</span><span className="font-serif text-3xl text-yara-wine">{formatPrice(total)}</span></div>
          <Link to="/checkout" className="btn-primary mt-7 w-full">Secure checkout <ArrowRight className="h-4 w-4" /></Link>
          <a href={createWhatsAppLink(cartOrderMessage(items, total))} target="_blank" rel="noreferrer" className="mt-3 inline-flex w-full min-h-11 items-center justify-center gap-2 rounded-full border border-[#20a852] px-5 py-3 text-xs font-semibold uppercase tracking-[0.1em] text-[#168a43]"><MessageCircle className="h-4 w-4" /> Order on WhatsApp</a>
