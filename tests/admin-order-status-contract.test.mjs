...
const orderAction = fs.readFileSync("src/modules/admin/order-actions.ts", "utf8");
...
test("admin order status RPC contract remains aligned with every payment state exposed by the form", () => {
  for (const value of [
    ...
    assert.match(orderAction, new RegExp(`"${value}"`));
  }
  assert.match(
...
test("admin fulfilment fields exist before the status action saves them", () => {
  ...
  assert.match(orderAction, /tracking_url: parsed\.data\.tracking_url/);
  assert.match(orderAction, /estimated_delivery_date: parsed\.data\.estimated_delivery_date/);
});
...
test("order admin routes do not eagerly load the native image processor", () => {
  ...
  assert.doesNotMatch(orderAction, /product-images|sharp/);
});
